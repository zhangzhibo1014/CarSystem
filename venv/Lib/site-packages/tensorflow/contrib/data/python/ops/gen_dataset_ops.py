...
_ops.RegisterShape("AssertNextDataset")(None)
...
def csv_dataset(filenames, compression_type, buffer_size, header, field_delim, use_quote_delim, na_value, select_cols, record_defaults, output_shapes, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    _, _, _op = _op_def_lib._apply_op_helper(
        ...
        buffer_size=buffer_size, header=header, field_delim=field_delim,
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        name, _ctx._post_execution_callbacks, filenames, compression_type,
...
@tf_export('identity_indexed_dataset')
def identity_indexed_dataset(size, name=None):
  r"""TODO: add doc.
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    _, _, _op = _op_def_lib._apply_op_helper(
        "IdentityIndexedDataset", size=size, name=name)
    _result = _op.outputs[:]
    _inputs_flat = _op.inputs
    _attrs = None
    _execute.record_gradient(
      "IdentityIndexedDataset", _inputs_flat, _attrs, _result, name)
    _result, = _result
    return _result
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name,
        "IdentityIndexedDataset", name, _ctx._post_execution_callbacks, size)
      return _result
    except _core._FallbackException:
      return identity_indexed_dataset_eager_fallback(
...
def identity_indexed_dataset_eager_fallback(size, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function identity_indexed_dataset
  """
  _ctx = ctx if ctx else _context.context()
  size = _ops.convert_to_tensor(size, _dtypes.uint64)
  _inputs_flat = [size]
  _attrs = None
  _result = _execute.execute(b"IdentityIndexedDataset", 1,
                             inputs=_inputs_flat, attrs=_attrs, ctx=_ctx,
                             name=name)
  _execute.record_gradient(
      "IdentityIndexedDataset", _inputs_flat, _attrs, _result, name)
...
_ops.RegisterShape("IdentityIndexedDataset")(None)
...
@tf_export('indexed_dataset_get')
def indexed_dataset_get(materialized, index, output_types, output_shapes, name=None):
  r"""Gets the element at `index` from `materialized` IndexedDataset.
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    if not isinstance(output_types, (list, tuple)):
      raise TypeError(
          "Expected list for 'output_types' argument to "
          "'indexed_dataset_get' Op, not %r." % output_types)
    output_types = [_execute.make_type(_t, "output_types") for _t in output_types]
    if not isinstance(output_shapes, (list, tuple)):
      raise TypeError(
          "Expected list for 'output_shapes' argument to "
          "'indexed_dataset_get' Op, not %r." % output_shapes)
    output_shapes = [_execute.make_shape(_s, "output_shapes") for _s in output_shapes]
    _, _, _op = _op_def_lib._apply_op_helper(
        "IndexedDatasetGet", materialized=materialized, index=index,
        output_types=output_types, output_shapes=output_shapes, name=name)
    _result = _op.outputs[:]
    if not _result:
      return _op
    ...
    _attrs = ("output_types", _op.get_attr("output_types"), "output_shapes",
              _op.get_attr("output_shapes"))
    _execute.record_gradient(
      "IndexedDatasetGet", _inputs_flat, _attrs, _result, name)
    return _result
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name,
        "IndexedDatasetGet", name, _ctx._post_execution_callbacks,
        materialized, index, "output_types", output_types, "output_shapes",
        output_shapes)
      return _result
    except _core._FallbackException:
      return indexed_dataset_get_eager_fallback(
...
def indexed_dataset_get_eager_fallback(materialized, index, output_types, output_shapes, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function indexed_dataset_get
  """
  _ctx = ctx if ctx else _context.context()
  if not isinstance(output_types, (list, tuple)):
    raise TypeError(
  ...
  _attrs = ("output_types", output_types, "output_shapes", output_shapes)
  _result = _execute.execute(b"IndexedDatasetGet", len(output_types),
                             inputs=_inputs_flat, attrs=_attrs, ctx=_ctx,
                             name=name)
  _execute.record_gradient(
      "IndexedDatasetGet", _inputs_flat, _attrs, _result, name)
...
_ops.RegisterShape("IndexedDatasetGet")(None)


@tf_export('indexed_dataset_materialize')
def indexed_dataset_materialize(dataset, materialized, name=None):
  r"""TODO: add doc.
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    _, _, _op = _op_def_lib._apply_op_helper(
        "IndexedDatasetMaterialize", dataset=dataset,
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name,
        "IndexedDatasetMaterialize", name, _ctx._post_execution_callbacks,
        dataset, materialized)
      return _result
    except _core._FallbackException:
      return indexed_dataset_materialize_eager_fallback(
...
def indexed_dataset_materialize_eager_fallback(dataset, materialized, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function indexed_dataset_materialize
  """
  _ctx = ctx if ctx else _context.context()
  ...
  _attrs = None
  _result = _execute.execute(b"IndexedDatasetMaterialize", 0,
                             inputs=_inputs_flat, attrs=_attrs, ctx=_ctx,
                             name=name)
  _result = None
  return _result

_ops.RegisterShape("IndexedDatasetMaterialize")(None)
...
def lmdb_dataset(filenames, output_types, output_shapes, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name, "LMDBDataset",
        name, _ctx._post_execution_callbacks, filenames, "output_types",
        output_types, "output_shapes", output_shapes)
      return _result
    except _core._FallbackException:
...
_ops.RegisterShape("LMDBDataset")(None)


@tf_export('materialized_index_dataset_handle')
def materialized_index_dataset_handle(container, shared_name, output_types, output_shapes, name=None):
  r"""TODO: add doc.
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    if not isinstance(output_types, (list, tuple)):
      raise TypeError(
          "Expected list for 'output_types' argument to "
          "'materialized_index_dataset_handle' Op, not %r." % output_types)
    output_types = [_execute.make_type(_t, "output_types") for _t in output_types]
    if not isinstance(output_shapes, (list, tuple)):
      raise TypeError(
          "Expected list for 'output_shapes' argument to "
          "'materialized_index_dataset_handle' Op, not %r." % output_shapes)
    output_shapes = [_execute.make_shape(_s, "output_shapes") for _s in output_shapes]
    _, _, _op = _op_def_lib._apply_op_helper(
        "MaterializedIndexDatasetHandle", container=container,
    ...
    _attrs = ("container", _op.get_attr("container"), "shared_name",
              ...
              _op.get_attr("output_shapes"))
    _execute.record_gradient(
      "MaterializedIndexDatasetHandle", _inputs_flat, _attrs, _result, name)
    _result, = _result
    return _result
...
def materialized_index_dataset_handle_eager_fallback(container, shared_name, output_types, output_shapes, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function materialized_index_dataset_handle
  """
  _ctx = ctx if ctx else _context.context()
  container = _execute.make_str(container, "container")
  shared_name = _execute.make_str(shared_name, "shared_name")
  ...
  _result = _execute.execute(b"MaterializedIndexDatasetHandle", 1,
                             inputs=_inputs_flat, attrs=_attrs, ctx=_ctx,
                             name=name)
  _execute.record_gradient(
      "MaterializedIndexDatasetHandle", _inputs_flat, _attrs, _result, name)
...
_ops.RegisterShape("MaterializedIndexDatasetHandle")(None)


@tf_export('multi_device_iterator')
...
_ops.RegisterShape("MultiDeviceIterator")(None)


@tf_export('multi_device_iterator_from_string_handle')
def multi_device_iterator_from_string_handle(string_handle, output_types=[], output_shapes=[], name=None):
  ...
  Returns:
    A `Tensor` of type `resource`. A MultiDeviceIterator resource.
  """
  _ctx = _context._context
  if _ctx is None or not _ctx._eager_context.is_eager:
...
_ops.RegisterShape("MultiDeviceIteratorFromStringHandle")(None)


@tf_export('multi_device_iterator_get_next_from_shard')
...
_ops.RegisterShape("MultiDeviceIteratorGetNextFromShard")(None)


@tf_export('multi_device_iterator_init')
...
_ops.RegisterShape("MultiDeviceIteratorInit")(None)


@tf_export('multi_device_iterator_to_string_handle')
def multi_device_iterator_to_string_handle(multi_device_iterator, name=None):
  ...
  Returns:
    A `Tensor` of type `string`. A string representing the resource.
  """
  _ctx = _context._context
  if _ctx is None or not _ctx._eager_context.is_eager:
    _, _, _op = _op_def_lib._apply_op_helper(
...
_ops.RegisterShape("MultiDeviceIteratorToStringHandle")(None)
...
# }
# op {
#   name: "IndexedDatasetMaterialize"
#   input_arg {
#     name: "dataset"
#     type: DT_VARIANT
#   }
...
# }
# op {
#   name: "MaterializedIndexDatasetHandle"
#   output_arg {
#     name: "handle"
#     type: DT_RESOURCE
#   }
