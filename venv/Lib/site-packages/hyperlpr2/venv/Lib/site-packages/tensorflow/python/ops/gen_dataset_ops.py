...
def window_dataset(input_dataset, window_size, output_types, output_shapes, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    output_shapes = [_execute.make_shape(_s, "output_shapes") for _s in output_shapes]
    _, _, _op = _op_def_lib._apply_op_helper(
        "WindowDataset", input_dataset=input_dataset, window_size=window_size,
        output_types=output_types, output_shapes=output_shapes, name=name)
    _result = _op.outputs[:]
    _inputs_flat = _op.inputs
    _attrs = ("output_types", _op.get_attr("output_types"), "output_shapes",
    ...
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        window_size, "output_types", output_types, "output_shapes",
        output_shapes)
      return _result
    except _core._FallbackException:
      return window_dataset_eager_fallback(
          input_dataset, window_size, output_types=output_types,
          output_shapes=output_shapes, name=name, ctx=_ctx)
    except _core._NotOkStatusException as e:
      if name is not None:
        message = e.message + " name: " + name
...
def window_dataset_eager_fallback(input_dataset, window_size, output_types, output_shapes, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  ...
  input_dataset = _ops.convert_to_tensor(input_dataset, _dtypes.variant)
  window_size = _ops.convert_to_tensor(window_size, _dtypes.int64)
  _inputs_flat = [input_dataset, window_size]
  _attrs = ("output_types", output_types, "output_shapes", output_shapes)
