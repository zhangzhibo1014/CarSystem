...
imp.reload(sys)
fontC = ImageFont.truetype("./Font/platech.ttf", 80, 0);
...
def drawRectBox(image, rect, addText):
    ...
    draw.text((int(rect[0] + 1), int(rect[1] - 16)), addText, (191, 255, 0), font=fontC)
    imagex = np.array(img)
...
def SimpleRecognizePlate(image):
    ...
    for j, plate in enumerate(images):
        ...
        res1, confidence1 = e2e.recognizeOne(image_rgb)
        #'''
        if res1 in result.keys() == True:
            if confidence1 > result[res1]:
                result[res1] = confidence1
            else:
                pass
        result[res1] = confidence1
        #'''
        ...
        if len(val) == 3:
            ...
            if confidence / 7 > 0.7:
                ...
                if res in result.keys() == True:
                    if confidence/7 > result[res]:
                        result[res] = confidence/7
                    else:
                        pass
                result[res] = confidence/7
                res_set.append(res)
    ...
    palte_key  = resultOfEnd(result)
    #return image, res_set
    return res_set, palte_key, image

def resultOfEnd(result):
    dict  = {}
    dict = result
    #print(dict)
    model = -1;
    ...
    for key in dict.keys():
        if (dict[key] > model) and (len(key) == 7):
            result_key = key
            result_value = dict[key]
            model = result_value
